...
public class Panel {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if(Booleans.clickuistyle.equalsIgnoreCase("New")){
			Gui.drawRect(x, y, x + width+3, y + height, 0xff121212);
		...
		}else if(Booleans.clickuistyle.equalsIgnoreCase("JellyLike")){
			Gui.drawRect(x-5, y, x + width+6, y + height, 0xff121212);
			
			double ex = x+6;
			Gui.drawRect(ex + 4,			y + 2, ex + 4.3, 		y + height - 2, 0xffaaaaaa);
			Gui.drawRect(ex - 4 + width, y + 2, ex - 4.3 + width, y + height - 2, 0xffaaaaaa);
			FontUtil.drawTotalCenteredStringWithShadow(title, x + width / 2, y + height / 2, 0xffefefef);
		}
		
		if (this.extended && !Elements.isEmpty()) {
			double startY = y + height;
			int epanelcolor = Booleans.clickuistyle.equalsIgnoreCase("New") ? 0xff232323 : Booleans.clickuistyle.equalsIgnoreCase("JellyLike") ? 0xbb151515 : 0;;
