...
public class ElementSlider extends Element {
	 ...
	 * Konstrukor
	 */
	public ElementSlider(ModuleButton iparent, Config iset) {
		parent = iparent;
		set = iset;
		dragging = false;
		super.setup();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		double percentBar = (set.doubleValue - set.doubleMin)/(set.doubleMax - set.doubleMin);
		 ...
		 */
		if (this.dragging) {
			double diff = set.doubleMax - set.doubleMin;
			double val = set.doubleMin + (MathHelper.clamp_double((mouseX - x) / width, 0, 1)) * diff;
			set.doubleValue = val; //Die Value im Setting updaten
		}
