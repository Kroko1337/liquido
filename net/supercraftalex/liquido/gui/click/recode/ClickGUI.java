package net.supercraftalex.liquido.gui.click.recode;
...
public class ClickGUI extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		FontUtil.drawStringWithShadow("by HeroCode edited by super_craft_alex", -Minecraft.getMinecraft().fontRendererObj.getStringWidth("by HeroCode edited by super_craft_alex"), -Minecraft.getMinecraft().fontRendererObj.FONT_HEIGHT, 0xff11F86B);
		GL11.glPopMatrix();
		 ...
		 */
		for (Panel panel : Liquido.INSTANCE.panels) {
				...
				for (ModuleButton b : panel.Elements) {
					if (b.extended && b.menuelements != null && !b.menuelements.isEmpty()) {
						...
						for (Element e : b.menuelements) {
							e.offset = off;
							e.update();
							if(Booleans.clickuistyle.equalsIgnoreCase("New")){
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
		 ...
		 */
		for (Panel panel : Liquido.INSTANCE.rpanels) {
			if (panel.extended && panel.visible && panel.Elements != null) {
				for (ModuleButton b : panel.Elements) {
					if (b.extended) {
						for (Element e : b.menuelements) {
		 ...
		 */
		for (Panel p : Liquido.INSTANCE.rpanels) {
	...
	public void initGui() {
		 ...
		 * Start blur
		 */
		if (OpenGlHelper.shadersSupported && mc.thePlayer instanceof EntityPlayer) {
